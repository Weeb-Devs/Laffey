...
module.exports = {
    ...
    async execute(message, args, client) {
        ...
        if (!args[0]) return message.channel.send(new handler().normalEmbed(`**${player.pitch}x**`))
        else if (args[0].toLowerCase() === 'reset') {
            player.setPitch(1);
            message.react('👌').catch((_) => {})
            return client.playerHandler.savePlayer(client.player.players.get(message.guild.id))
        } else {
            if (isNaN(args[0]) || Number(args[0]) > 5 || Number(args[0]) < 0) return message.channel.send(new handler().normalEmbed(`Pitch must be a number and between 0 and 5`))
