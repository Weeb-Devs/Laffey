...
module.exports = {
    ...
    async execute(message, args, client) {
        ...
        if (!args[0]) return message.channel.send(new handler().normalEmbed(`**${player.speed}x**`))
        else if (args[0].toLowerCase() === 'reset') {
            player.setSpeed(1);
            message.react('👌').catch((_) => {})
            return client.playerHandler.savePlayer(client.player.players.get(message.guild.id))
        } else {
            if (isNaN(args[0]) || Number(args[0]) > 5 || Number(args[0]) < 0) return message.channel.send(new handler().normalEmbed(`Speed must be a number and between 0 and 5`))
